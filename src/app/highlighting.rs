    ...
    pub fn dark() -> Self {
        ...
        Self {
            ...
            formats: enum_map::enum_map![
                ...
                TokenType::Function => TextFormat::simple(font_id.clone(),Color32::from_rgb(220,160,20)),
            ],
        }
    }
    ...
    pub fn light() -> Self {
        ...
        Self {
            ...
            formats: enum_map::enum_map![
                ...
                TokenType::Keyword => TextFormat::simple(font_id.clone(), Color32::LIGHT_RED),
                TokenType::Literal => TextFormat::simple(font_id.clone(), Color32::DARK_GRAY),
                TokenType::StringLiteral => TextFormat::simple(font_id.clone(), Color32::from_rgb(37, 203, 105)),
                TokenType::Punctuation => TextFormat::simple(font_id.clone(), Color32::LIGHT_RED),
                TokenType::Whitespace => TextFormat::simple(font_id.clone(), Color32::TRANSPARENT),
                TokenType::Function => TextFormat::simple(font_id.clone(),Color32::from_rgb(220,160,20)),
            ],
        }
            ...
            ui.vertical(|ui| {
                ...
                ui.scope(|ui| {
                    for (tt, tt_name) in [
                        ...
                        (TokenType::Punctuation, "function ;"),
...
impl Highlighter {
    ...
    fn highlight_impl(
        ...
        while !text.is_empty() {
            if text.starts_with(language.comments.as_str()) {
                let end = text.find('\n').unwrap_or(text.len());
                ...
                text = &text[end..];
            } else if text.starts_with(|c: char| c == '.' || c == ':') {
                let mut end = text[1..]
                    .find(|c: char| c.is_ascii_alphabetic() || c == '_')
                    .map_or_else(|| text.len(), |i| i + 1);
                job.append(&text[..end], 0.0, theme.formats[TokenType::Literal].clone());
                text = &text[end..];
                if end == 1 && !text.is_empty() {
                    end = text[1..]
                        .find(|c: char| !c.is_ascii_alphanumeric() && c != '_')
                        .map_or_else(|| text.len(), |i| i + 1);
                    let word = &text[..end];
                    job.append(word, 0.0, theme.formats[TokenType::Function].clone());
                    text = &text[end..];
                }
                ...
                let tt = if language.is_keyword(word) {
                    TokenType::Keyword
                } else if text.starts_with(|c: char| c == '(') {
                    TokenType::Function
                } else {
                    TokenType::Literal
                };
